...
impl Cmd {
    pub async fn run(&self, settings: &Settings) -> Result<()> {
        ...
        let username: String = or_user_input(&self.username, "username");
        let key: String = or_user_input(&self.key, "encryption key [blank to use existing key file]");
        let password: String = self.password.clone().unwrap_or_else(read_user_password);

        let key_path: &str = settings.key_path.as_str();
        if key.is_empty() {
            if PathBuf::from(key_path).exists() {
                let bytes: String = fs_err::read_to_string(key_path)
            ...
            let key: String = match bip39::Mnemonic::from_phrase(&key, bip39::Language::English) {
            ...
            let mut file: File = File::create(key_path).await?;
            file.write_all(key.as_bytes()).await?;
        }

        let session: atuin_common::api::LoginResponse = api_client::login(
        ...
        let session_path: &str = settings.session_path.as_str();
        let mut file: File = File::create(session_path).await?;
...
pub(super) fn read_user_password() -> String {
    let password: std::prelude::v1::Result<String, io::Error> = prompt_password("Please enter password: ");
...
mod tests {
    ...
    fn mnemonic_round_trip() {
        ...
        let phrase: String = bip39::Mnemonic::from_entropy(&key, bip39::Language::English)
            .unwrap()
            .into_phrase();
        let mnemonic: bip39::Mnemonic = bip39::Mnemonic::from_phrase(&phrase, bip39::Language::English).unwrap();
