    ...
    pub async fn run(
        ...
        match self {
            ...
            Self::Key { base64 } => {
                ...
                if base64 {
                    let encode: String = encode_key(&key).wrap_err("could not encode encryption key")?;
                    println!("{encode}");
                } else {
                    let mnemonic: bip39::Mnemonic = bip39::Mnemonic::from_entropy(&key, bip39::Language::English)
